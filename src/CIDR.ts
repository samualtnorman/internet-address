...
export type Prefix<T extends IPv4 | IPv6> = T extends IPv4 ? IPv4.Prefix : IPv6.Prefix
export type CIDR<T extends IPv4 | IPv6> = { address: T, prefix: Prefix<T> }
export type RangeList<TRange extends string, TIP extends IPv4 | IPv6> = { [K in TRange]?: CIDR<TIP>[] }

export const toString = <T extends IPv4 | IPv6>(cidr: CIDR<T>): string =>
	`${cidr.address instanceof Uint8Array ? ipv4ToString(cidr.address) : ipv6ToString(cidr.address)}/${cidr.prefix}`

export const from = <T extends IPv4 | IPv6>(address: T, prefix: Prefix<T>): CIDR<T> => ({ address, prefix })
...
if (import.meta.vitest) {
	...
	test(`from()`, () => {
		...
		expect(ipv4Cidr).toStrictEqual<CIDR<IPv4>>({ address: ipv4, prefix: 31 })
		...
		expect(ipv6Cidr).toStrictEqual<CIDR<IPv6>>({ address: ipv6, prefix: 128 })
